...
func reportMetrics(ctx context.Context, rt *runtime.Runtime, dbWaitDuration, redisWaitDuration *time.Duration) (int, error) {
	...
	*redisWaitDuration = redisStats.WaitDuration
	...
	// nanoRP: the spool doesn't exist when Dynamo is disabled
	if rt.Dynamo.Enabled() {
		metrics = append(metrics, cwatch.Datum("DynamoSpooledItems", float64(rt.Dynamo.Spool.Size()), cwtypes.StandardUnitCount))
	}
