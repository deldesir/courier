...
func (h *handler) receiveMessage(ctx context.Context, channel *models.Channel, r *http.Request, payload *receivePayload, in *channels.Received, clog *models.ChannelLog) error {
	...
	// nanoRP: with S3 disabled the upload endpoint can't have stored anything, so no attachment reference can
	// be valid - and there's no storage URL prefix to compute (an empty prefix would match every URL)
	prefix := ""
	if len(payload.Attachments) > 0 {
		if h.Runtime().S3 == nil {
			return fmt.Errorf("invalid attachment: %s", payload.Attachments[0])
		}
